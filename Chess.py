...
class Board(tk.Frame):
    ...
    def import_pieces(self):  # Opens and stores images of pieces and prepares the pieces for the game for both sides
        path = os.path.join(os.path.dirname(__file__), "White")  # Stores white piece images into dicts
        w_dirs = os.listdir(path)
        for file in w_dirs:
            img = Image.open(os.path.join(path, file))
            img = img.resize((80, 80), Image.Resampling.LANCZOS)  # Use Image.Resampling.LANCZOS directly
            img = ImageTk.PhotoImage(image=img)
            self.white_images.setdefault(file, img)

        path = os.path.join(os.path.dirname(__file__), "Black")  # Stores black piece images into dicts
